...
/* ────────────────────────────────────────────────────────
   ...
   No GameFunnel dependency for rendering. 
   State updates are fire-and-forget via localStorage.
   ──────────────────────────────────────────────────────── */
...
const CALL_SECONDS = 15

function updateFunnelState(patch: (s: Record<string, unknown>) => Record<string, unknown>) {
  try {
    const raw = localStorage.getItem("cidade-neon-funnel-v2")
    if (!raw) return
    const state = JSON.parse(raw)
    const patched = patch(state)
    localStorage.setItem("cidade-neon-funnel-v2", JSON.stringify({ ...patched, lastVisitedAt: Date.now() }))
  } catch { /* noop */ }
}

export default function DBeeCallPage() {
  /* purely local state - no context dependency */
  const [step, setStep] = useState<"ring" | "active" | "transition">("ring")
  ...
  useEffect(() => {
    ...
    const t = setTimeout(() => {
      ...
      updateFunnelState((s) => ({
        ...s,
        cinematicStep: "active-call",
        perAppState: { ...(s.perAppState as Record<string, unknown>), call: { duration: 0, hackerProgress: 0, answered: true } },
      }))
    }, 1200)
  ...
  useEffect(() => {
    ...
    timer.current = setInterval(() => {
      setSec((prev) => {
        ...
        // terminal lines
        setVisLines(Math.min(Math.floor((next / CALL_SECONDS) * HACKER_LINES.length) + 1, HACKER_LINES.length))

        // random glitch
        if (next > 3 && Math.random() > 0.6) {
          setGlitch(true)
        ...
        if (next >= CALL_SECONDS) {
          ...
          updateFunnelState((s) => ({
            ...
            perAppState: { ...(s.perAppState as Record<string, unknown>), hacker: { progress: 0, lines: [], completed: false } },
          }))
          setTimeout(() => { window.location.href = "/hacker" }, 600)
        }
      ...
      >
        ...
        >
          <div className="p-4 pt-20 font-mono text-xs leading-6 text-green-500 select-none">
            {HACKER_LINES.slice(0, visLines).map((line, i) => (
              <div
                key={i}
                className="opacity-0"
                style={{
        ...
        <div className="relative z-10 flex-1 flex flex-col items-center pt-8">
          <div className="relative mb-3">
            <div className={`w-[110px] h-[110px] rounded-full overflow-hidden ring-[3px] ring-white/20 ${step === "ring" ? "animate-pulse" : ""}`}>
              <Image src="/images/avatar-dbee.jpg" alt="D-Bee" width={110} height={110} className="w-full h-full object-cover" priority />
            </div>
            {step === "ring" && (
          ...
          {step === "ring" && <p className="text-[#34C759] text-[15px] mt-3 animate-pulse">Conectando...</p>}
          {step === "active" && <p className="text-white text-[20px] font-light mt-3 tabular-nums tracking-wider">{fmt(sec)}</p>}
          {step === "transition" && <p className="text-red-500 text-[15px] mt-3 animate-pulse font-mono">CONEXAO COMPROMETIDA</p>}
        </div>
          ...
          {step === "active" ? (
            <>
              <div className="grid grid-cols-3 gap-3 mb-6">
                {[
                  { label: "silenciar", d: "M5.586 15H4a1 1 0 01-1-1v-4a1 1 0 011-1h1.586l4.707-4.707C10.923 3.663 12 4.109 12 5v14c0 .891-1.077 1.337-1.707.707L5.586 15zM17 14l2-2m0 0l2-2m-2 2l-2-2m2 2l2 2", stroke: true },
                  { label: "alto-falante", d: "M3 9v6h4l5 5V4L7 9H3zm13.5 3c0-1.77-1.02-3.29-2.5-4.03v8.05c1.48-.73 2.5-2.25 2.5-4.02zM14 3.23v2.06c2.89.86 5 3.54 5 6.71s-2.11 5.85-5 6.71v2.06c4.01-.91 7-4.49 7-8.77s-2.99-7.86-7-8.77z", stroke: false },
                  { label: "teclado", d: "M12 7a2 2 0 100-4 2 2 0 000 4zm0 6a2 2 0 100-4 2 2 0 000 4zm0 6a2 2 0 100-4 2 2 0 000 4z", stroke: false },
                ].map((b) => (
                  <div key={b.label} className="flex flex-col items-center gap-1.5">
                    <div className="w-[60px] h-[60px] rounded-full bg-white/10 flex items-center justify-center">
                      <svg className="w-6 h-6 text-white" fill={b.stroke ? "none" : "currentColor"} stroke={b.stroke ? "currentColor" : "none"} strokeWidth={b.stroke ? 2 : 0} viewBox="0 0 24 24"><path d={b.d} strokeLinecap="round" strokeLinejoin="round" /></svg>
                    </div>
                    <span className="text-white/70 text-[11px]">{b.label}</span>
                  </div>
                ))}
              </div>
              <div className="flex justify-center">
                <button
                  ...
                  onClick={() => {
                    if (timer.current) clearInterval(timer.current)
                    setStep("transition")
                    updateFunnelState((s) => ({ ...s, cinematicStep: "hacker-takeover" }))
                    setTimeout(() => { window.location.href = "/hacker" }, 400)
                  }}
